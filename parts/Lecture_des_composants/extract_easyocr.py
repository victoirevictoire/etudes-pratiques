...
def _c(text, *codes):
    """Entoure `text` avec les codes ANSI `codes` et remet à zéro en fin."""
    return ''.join(codes) + str(text) + C.RESET

def step(icon, label, value=''):
    """Affiche une ligne de log structurée : icône + label en gras + valeur grise."""
    val = f'  {C.GRAY}{value}{C.RESET}' if value else ''
    print(f'  {icon}  {C.BOLD}{label}{C.RESET}{val}')

def ok(label):
    """Affiche une ligne de succès avec coche verte."""
    print(f'  {_c("✔", C.GREEN)}  {label}')

def progress_bar(current, total, width=30):
    """
    Affiche une barre de progression sur la même ligne (écrasement via \\r).

    Args:
        current: Nombre d'éléments traités jusqu'ici.
        total:   Nombre total d'éléments.
        width:   Largeur en caractères de la barre.
    """
...
def fix_ocr_confusion(text: str) -> str:
    ...
    - Cas spécial : "RI2"→"R12", "CI8"→"C18" (I en fin de préfixe = 1 mal lu)
    Exemples : "C41J"→"C411", "R51G"→"R516", "0C402"→"OC402", "R516A"→"R516A"
               "RI2"→"R12", "CI8"→"C18", "CI402"→"C1402"
    """
    if not text:
        return text
    ...
    # Cas spécial : préfixe se termine par 'I' précédé d'une seule lettre
    # → "RI" = R + confusion I/1, "CI" = C + confusion I/1
    # Préfixes standards à 2 lettres légitimes : IC, TR, VR, BC, ZD, SW, TP, FR
    ...
    # Suffixe : dernier char lettre NON présent dans DIGIT_FIXES → suffixe réel
    # ex : "R516A" → suffix "A" (valide)  |  "R51G" → G est dans DIGIT_FIXES → digit
...
def preprocess_crop(crop_bgr, scale=5):
    """
    Prépare un crop pour EasyOCR : upscale + netteté + CLAHE + marge blanche.

    Le pipeline est : redimensionnement cubique → filtre laplacien d'accentuation
    → niveaux de gris → égalisation adaptative CLAHE → reconversion BGR
    → bordure blanche de 10 px (évite que le texte touche les bords).

    Args:
        crop_bgr: Image BGR extraite du schéma (numpy array).
        scale:    Facteur d'agrandissement appliqué avant l'OCR.

    Returns:
        Image BGR prête pour EasyOCR, plus grande et à contraste amélioré.
    """
    h, w = crop_bgr.shape[:2]
    big  = cv2.resize(crop_bgr, (w * scale, h * scale),
                      interpolation=cv2.INTER_CUBIC)

    # Filtre de netteté laplacien : accentue les contours des caractères
...
def is_valid_ref(text: str) -> bool:
    """
    Valide qu'un texte respecte le format d'une référence de composant électronique.

    Formats reconnus : R516, C401A, IC402, BC178, FR409, TP12, St001, R12-3…

    Args:
        text: Texte brut à valider.

    Returns:
        True si le texte correspond à une référence valide, False sinon.
    """
...
def _run_easyocr(img_bgr):
    """
    Lance EasyOCR sur une image et retourne les candidats filtrés par confiance.

    Utilise l'allowlist pour limiter les caractères reconnus aux lettres,
    chiffres, tiret et underscore — réduit les faux positifs sur les symboles
    graphiques présents dans les schémas PCB.

    Args:
        img_bgr: Image BGR (numpy array) à analyser, typiquement un crop prétraité.

    Returns:
        Liste de tuples (texte, confiance) triée par confiance décroissante.
        Retourne [] en cas d'erreur EasyOCR.
    """
    try:
...
def preprocess_crop_otsu(crop_bgr, scale=5):
    """
    Variante de prétraitement par binarisation Otsu.

    Préférable à CLAHE quand le fond est sombre ou le contraste inversé :
    Otsu calcule automatiquement le seuil optimal pour séparer texte et fond.

    Args:
        crop_bgr: Image BGR brute du crop YOLO.
        scale:    Facteur d'agrandissement avant binarisation.

    Returns:
        Image BGR binarisée (noir/blanc) avec marge blanche de 10 px.
    """
    h, w = crop_bgr.shape[:2]
    big  = cv2.resize(crop_bgr, (w * scale, h * scale),
                      interpolation=cv2.INTER_CUBIC)
...
def run_ocr(crop_bgr, scale=5):
    ...
    """
    def best_from(img):
        """Retourne (texte_corrigé, is_valid, conf) pour le meilleur candidat OCR de `img`."""
...
def upscale_image(img, factor: int):
    """
    Agrandit l'image entière par interpolation cubique puis accentue les contours.

    Appelé avant YOLO quand le DPI estimé est trop bas (< 150 DPI) pour que
    les crops extraits aient suffisamment de pixels pour l'OCR.

    Args:
        img:    Image BGR source (numpy array).
        factor: Facteur d'agrandissement entier (ex. 2 ou 3).

    Returns:
        Image BGR agrandie et accentuée.
    """
    h, w = img.shape[:2]
    big  = cv2.resize(img, (w * factor, h * factor),
                      interpolation=cv2.INTER_CUBIC)
    k    = np.array([[0, -1, 0], [-1, 5, -1], [0, -1, 0]])
...
def auto_params(img):
    ...
    Adapte : pre_upscale (si DPI faible), imgsz YOLO, scale crop, conf,
    ...
    # Estimation DPI (hypothèse page A4 paysage ~297mm de large)
    dpi_est = int(w / (297 / 25.4))

    # Pré-upscale automatique si image trop petite
    # → agrandit AVANT YOLO pour que les crops soient plus grands
    if dpi_est < 80:
        pre_upscale = 3    # ex: 427×266 → 1281×798
    elif dpi_est < 150:
        pre_upscale = 2    # ex: 708×984 → 1416×1968
    else:
        pre_upscale = 1    # image déjà assez grande, pas d'upscale

    # Dimensions effectives après pré-upscale
    eff_w, eff_h = w * pre_upscale, h * pre_upscale
    eff_px = eff_w * eff_h

    # imgsz YOLO + scale crop selon résolution EFFECTIVE
    if eff_px > 3_000_000:
        imgsz, scale = 1280, 5
    elif eff_px > 1_000_000:
        imgsz, scale = 1024, 6
    elif eff_px > 300_000:
        imgsz, scale = 640, 8
    else:
    ...
    # Affichage
    if dpi_est >= 200:
        qual_tag = _c('● BONNE', C.GREEN)
    elif dpi_est >= 120:
        qual_tag = _c('● MOYENNE', C.YELLOW)
    else:
        qual_tag = _c('● FAIBLE', C.RED)

    up_tag = (f'  {_c(f"→ pré-upscale ×{pre_upscale} ({eff_w}×{eff_h}px)", C.CYAN)}'
              if pre_upscale > 1 else '')
    inv_tag = f'  {_c("⚠ fond sombre → inversion", C.YELLOW)}' if invert else ''

    print(f'  {_c("◈", C.CYAN)}  {_c("IMAGE", C.BOLD)}  '
          f'{_c(f"{w}×{h} px", C.WHITE)}  '
          f'{_c(f"~{dpi_est} DPI", C.CYAN)}  '
          f'{qual_tag}'
          f'{up_tag}'
          f'{inv_tag}')
    print(f'  {_c("◈", C.CYAN)}  {_c("PARAMS", C.BOLD)}  '
          f'imgsz={_c(imgsz, C.GREEN)}  '
          f'scale={_c(scale, C.GREEN)}  '
          f'conf={_c(conf, C.GREEN)}  '
          f'contraste={_c(f"{std:.0f}", C.GRAY)}  '
          f'lum={_c(f"{mean:.0f}", C.GRAY)}')
    return params
...
def extract(img_path, model_path, conf_thresh=None):
    """
    Pipeline complet : charge l'image, détecte les zones (YOLO), lit le texte (OCR),
    corrige les erreurs et déduplique les références.

    Args:
        img_path:    Chemin vers l'image PCB (Path ou str).
        model_path:  Chemin vers le modèle YOLO `.pt` (Path ou str).
        conf_thresh: Seuil de confiance YOLO manuel. Si None, calculé
                     automatiquement par `auto_params`.

    Returns:
        Tuple (img, detections) où :
        - img        : image numpy BGR (éventuellement upscalée/inversée)
        - detections : liste de dicts, un par zone détectée, avec les clés
                       id, text, is_valid_ref, yolo_conf, ocr_conf, score,
                       bbox, center.

    Raises:
        FileNotFoundError: Si l'image n'existe pas ou ne peut pas être lue.
    """
    ...
    # Pré-upscale si image faible résolution
    if params["pre_upscale"] > 1:
        img = upscale_image(img, params["pre_upscale"])
        ok(f'Pré-upscale ×{params["pre_upscale"]} → {img.shape[1]}×{img.shape[0]} px')
...
def draw_results(img, detections):
    """
    Dessine les boîtes et labels sur une copie de l'image.

    Couleur des boîtes :
    - Vert  (0, 200, 0)   → référence valide
    - Orange (0, 140, 255) → texte non reconnu

    Args:
        img:        Image BGR source (non modifiée).
        detections: Liste de dicts produite par `extract`.

    Returns:
        Nouvelle image BGR avec les annotations superposées.
    """
    out = img.copy()
    for d in detections:
        b = d["bbox"]
...
def save_json(detections, path):
    """
    Sauvegarde la liste des détections en JSON indenté (UTF-8).

    Args:
        detections: Liste de dicts produite par `extract`.
        path:       Chemin de sortie (Path ou str).
    """
...
def save_csv(detections, path):
    """
    Sauvegarde la liste des détections en CSV (compatible Excel / pandas).

    Colonnes : id, text, is_valid_ref, yolo_conf, ocr_conf, score,
               x1, y1, x2, y2, cx, cy.
...
def main():
    """
    Point d'entrée CLI — parse les arguments, lance le pipeline et affiche
    le résumé des résultats dans le terminal.
