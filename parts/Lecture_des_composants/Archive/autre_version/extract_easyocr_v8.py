...
def run_ocr(crop_bgr):
    ...
    pre_clahe = preprocess_crop(crop_bgr)
    ...
    pre_otsu = preprocess_crop_otsu(crop_bgr)
...
def extract(img_path, model_path, conf_thresh=0.3):
    ...
    yolo_result = model(img, conf=conf_thresh, imgsz=YOLO_IMGSZ, verbose=False)[0]
    raw_boxes   = yolo_result.boxes
    print(f"  {len(raw_boxes)} zones brutes YOLO  (conf >= {conf_thresh})")
    ...
    # Merge des boîtes voisines (corrige les refs splitées)
    boxes = merge_nearby_boxes(boxes_raw)
    print(f"  {len(boxes)} zones apres merge voisines "
          f"({len(boxes_raw) - len(boxes)} fusions)")
    ...
    for i, (x1, y1, x2, y2, conf_yolo) in enumerate(boxes):
        ...
        text, valid, ocr_conf = run_ocr(crop)
        detections.append({
            "id":           i + 1,
            "text":         text,
            "is_valid_ref": valid,
        ...
        if (i + 1) % 50 == 0:
            print(f"     {i+1}/{len(boxes)} crops traites...")
    ...
    if before != after:
        print(f"  Deduplication : {before} → {after} refs valides uniques")
...
def main():
    ...
    print(f"\n{'='*52}")
    print(f"  PCB Extractor v7 — EasyOCR + merge voisines")
    print(f"{'='*52}")
    ...
    print(f"\n{'-'*52}")
    print(f"  Zones traitees  : {len(detections)}")
    print(f"  Refs valides    : {len(valid)}")
    print(f"  Non reconnu     : {len(others)}")
    print(f"{'-'*52}")

    if valid:
        print(f"\nRefs trouvees (par score) :")
