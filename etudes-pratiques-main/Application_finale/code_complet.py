...
"""
code_complet.py
Lancement : python3 code_complet.py <document.pdf>
=======================
...
# lors du traitement des différentes pages et zones du document.
#
...
def run_ocr(crop_bgr, scale=5):
    """
    Pipeline OCR à 3 tentatives sur un crop issu d'une zone détectée par YOLO.
...
def run_ocr_on_crop(crop_bgr, scale=4):
    """
    Pipeline OCR à 3 tentatives sur un crop issu d'une image convertie depuis PDF
    à 300 DPI. Le scale par défaut de 4 est adapté à cette résolution source.
...
# YOLO peut détecter une référence comme deux boîtes séparées.
...
# Une même référence physique peut être détectée plusieurs fois sur la même image
# si elle est visible depuis deux angles légèrement différents, si le label est
# partiellement à cheval sur deux zones YOLO, ou si le modèle génère des boîtes
# redondantes non supprimées par NMS (NMS = Non-Maximum Suppression).
#
# Après OCR de toutes les zones, on construit un dictionnaire
...
def deduplicate_refs(detections):
    ...
    score composite (yolo_conf × ocr_conf).
...
def _deduplicate_refs_viewer(found_refs):
    """
    Déduplique les références en gardant la détection avec le meilleur score
    composite (yolo_conf × ocr_conf) par référence.
...
# Les paramètres optimaux de détection YOLO (imgsz, conf) et de prétraitement
# OCR (scale d'upscale, inversion fond sombre) varient selon les caractéristiques
# de l'image source. Une image 4K à fort contraste nécessite des paramètres
# différents d'une image 640px peu contrastée.
#
...
def extract(img_path, model_path, conf_thresh=None):
    """
    Extrait les références de composants d'une image via YOLO + EasyOCR.
...
#
# Deux modèles YOLO distincts sont utilisés dans le pipeline :
#   - Modèle de zones (best.pt) : détecte les grandes régions structurelles du
#     document (Table BOM, Schéma électronique, Carte PCB).
#     Utilisé avec YOLO_CONF_THRESHOLD = 0.212 (seuil bas pour ne pas rater une
#     zone unique par document).
#   - Modèle de références (best_composants.pt) : détecte les petites zones de
#     texte des références individuelles sur le schéma ou la carte.
#
...
# Ce pipeline orchestre les étapes entre le PDF brut et l'interface graphique.
# Pour chaque classe de zone (Table, Diagram, Board), seule la détection de plus
# grande surface est conservée parmi toutes les pages du document.
#
...
# La BOM présente les références de composants dans une colonne de gauche.
# La stratégie ici est géométrique plutôt que YOLO :
#
#   1. Isolation des 22% gauches de l'image. Ce ratio empirique correspond à la
#      largeur typique de la colonne "Référence" dans les BOM électroniques
#      japonaises et européennes de l'époque (années 70-80, format papier).
#
...
# Le modèle best_composants.pt est entraîné spécifiquement pour détecter les
# petits labels de références sur les schémas et cartes PCB issus du PDF.
# Il opère avec conf=0.20 pour maximiser le rappel dans l'interface interactive.
# La déduplication (_deduplicate_refs_viewer) conserve par référence la détection
# ayant le score composite le plus élevé (yolo_conf × ocr_conf).
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage:")
        print("python3 code_complet.py <document.pdf>")
        sys.exit(1)
