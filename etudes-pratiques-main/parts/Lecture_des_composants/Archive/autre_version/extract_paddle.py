...
extract_paddle.py
=================
Pipeline YOLO + PaddleOCR (nouvelle API 2.x/3.x).

Installation :
  pip install paddlepaddle paddleocr

Usage:
  python3 extract_paddle.py --img carte.png --model best.pt
"""
...
# ──────────────────────────────────────────────────────────
#  1. PADDLEOCR — nouvelle API
# ──────────────────────────────────────────────────────────

print("⏳ Chargement PaddleOCR...")
try:
    # Nouvelle API (paddleocr >= 2.8)
    OCR = PaddleOCR(
    ...
    USE_NEW_API = False
print(f"✅ PaddleOCR prêt (API {'nouvelle' if USE_NEW_API else 'ancienne'})")
...
# ──────────────────────────────────────────────────────────
#  5. OCR — gère ancienne et nouvelle API
# ──────────────────────────────────────────────────────────

def parse_paddle_result(result):
    """Extrait (text, conf) depuis le résultat PaddleOCR quelle que soit l'API."""
    items = []
    if not result:
        return items
    try:
        if USE_NEW_API:
            # Nouvelle API : result est une liste de dicts
            for r in result:
                texts  = r.get('rec_text', [])
                scores = r.get('rec_score', [])
                for t, s in zip(texts, scores):
                    items.append((str(t), float(s)))
        else:
            # Ancienne API : [[ [[box], (text, conf)], ... ]]
            for page in result:
                if not page:
                    continue
                for line in page:
...
def run_ocr(crop_bgr):
    ...
    raw_items = parse_paddle_result(result)
    if not raw_items:
        return None, False, 0.0
    ...
    for (text, conf) in raw_items:
        ...
        if text_clean:
            candidates.append((fix_ocr_confusion(text_clean), conf))
...
def extract(img_path, model_path, conf=0.3):
    ...
    print(f"  ⏳ OCR PaddleOCR en cours...")
...
def main():
    ...
    print(f"\n{'═'*50}")
    print(f"  PCB Extractor — PaddleOCR + correction")
    print(f"{'═'*50}")
    ...
    print(f"\n✅ Terminé ! Ouvre avec : xdg-open results/{stem}_{ts}_annotated.png\n")
