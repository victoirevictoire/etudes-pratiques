...
print(f'  {_c("ok", C.GREEN)}  EasyOCR pret  {_c(f"({time.time()-_t0:.1f}s)", C.GRAY)}')
...
def main():
    ...
    if not img_path.exists():
        print(f'  {_c("no", C.RED)}  Image introuvable : {args.img}'); return
    if not model_path.exists():
        print(f'  {_c("no", C.RED)}  Modele introuvable : {args.model}'); return
